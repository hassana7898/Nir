...
import { getInventoryStockByDate, recordInventoryTransaction, deleteTransactionsByReference } from './inventoryService';
...
export const deleteInvoiceWithTransaction = async (id: string): Promise<void> => {
  await db.transaction(async (tx: any) => {
    await tx.update(invoices).set({ deletedAt: new Date(), updatedAt: new Date() }).where(eq(invoices.id, id));
    await deleteTransactionsByReference(tx, id);
  });
};
