...
export const deleteTransactionsByReference = async (tx: any, referenceId: string): Promise<void> => {
  const affected = await tx.select({ productId: inventory_transactions.productId }).from(inventory_transactions).where(eq(inventory_transactions.referenceId, referenceId));
  await tx.delete(inventory_transactions).where(eq(inventory_transactions.referenceId, referenceId));
  for (const productId of Array.from(new Set(affected.map((a: any) => a.productId))) as string[]) {
